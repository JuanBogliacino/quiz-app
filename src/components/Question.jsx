...
const Question = ({ filteredQuestion, setIndexQuestion, indexQuestion, questionFiltered, setActiveQuiz }) => {
  ...
  return (
    ...
    {activeResults ? (
        ...
        <button className='border-2 border-gray-900 px-5 py-2 rounded-lg font-bold transition-all hover:bg-yellow-500 hover:border-yellow-500' onClick={onReset}>
            Reiniciar
        </button>
        <div>
            <h1 className="font-bold text-[15px] text-center">{filteredQuestion.question}</h1>
        </div>
            ...
            {/* Mapeamos un arreglo de respuesta correcto y respuestas incorrectas */}
            {
                answersRandom.map((answer, index) => (
                    <button
                     ...
                     disabled={answered}
                    >
                     <p className="font-medium text-center text-[12px] md:text-[14px]">
                      {answer}
                     </p>
                    </button>
                ))
        ...
        {
            indexQuestion + 1 === questionFiltered.length ? (
                <button className='text-white bg-gray-900 text-dark rounded-md px-5 py-2 hover:bg-yellow-500 hover:text-black font-medium'
                 onClick={() => {
                    setAnswered(false)
                ...
                <button className='text-white bg-gray-900 text-dark rounded-md px-5 py-2 hover:bg-yellow-500 hover:text-black font-medium' onClick={onNextQuestion} disabled={!answered}>
                Siguiente Pregunta
                </button>
            )
        }
